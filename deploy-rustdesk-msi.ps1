...
if ($rustdeskVersionOverride) {
    ...
    $release = Get-LatestRustDeskRelease -InstallerType 'msi'
    if (-not $release) {
...
if (-not $skipInstall) {
    ...
    try {
        Write-Host "Starting silent MSI installation..."
        Start-Process -FilePath "msiexec.exe" -ArgumentList "/i", "`"$installerPath`"", "/qn", "ADDLOCAL=ALL" -Wait -NoNewWindow
        Write-Host "Installation completed."
    } catch {
        Write-Host "ERROR: RustDesk MSI installation failed: $($_.Exception.Message)"
        exit 1
    }
